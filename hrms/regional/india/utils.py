...
def calculate_annual_eligible_hra_exemption(doc):
    ...
    if hra_component and basic_component:
        ...
        period_start_date = frappe.db.get_value(
            "Payroll Period", doc.payroll_period, "start_date"
        )
...
def calculate_tax_with_marginal_relief(tax_slab, tax_amount, annual_taxable_earning):
    ...
    if tax_slab.get("marginal_relief_limit"):
        ...
        if (
            annual_taxable_earning > tax_relief_limit
            and annual_taxable_earning < marginal_relief_limit
        ):
            income_excess_over_tax_relief = (
                annual_taxable_earning - tax_slab.tax_relief_limit
            )
