...
def get_attendance_for_calendar(
    employee: str, from_date: str, to_date: str
) -> list[dict[str, str]]:
...
def get_shift_request_approvers(employee: str) -> str | list[str]:
    ...
    if department:
        department_approvers = get_department_approvers(
    ...
    shift_request_approver_name = frappe.db.get_value(
        "User", shift_request_approver, "full_name", cache=True
    )
...
def get_leave_approval_details(employee: str) -> dict:
    ...
    leave_approver_name = frappe.db.get_value(
        "User", leave_approver, "full_name", cache=True
    )
    ...
    if leave_approver and leave_approver not in [
        ...
        department_approvers.append(
            {"name": leave_approver, "full_name": leave_approver_name}
        )
...
def get_department_approvers(department: str, parentfield: str) -> list[str]:
    ...
    department_approvers = (
        ...
        .where(
            (Approver.parent.isin(departments)) & (Approver.parentfield == parentfield)
        )
    ).run(as_dict=True)
...
def get_expense_claim_summary(employee: str) -> dict:
    ...
    approved_total_claimed_case = (
        frappe.qb.terms.Case()
        .when(Claim.approval_status == "Approved", Claim.total_claimed_amount)
        .else_(0)
    )
    sum_approved_total_claimed = Sum(approved_total_claimed_case).as_(
        "total_claimed_in_approved"
    )

    rejected_claims_case = (
        frappe.qb.terms.Case()
        .when(Claim.approval_status == "Rejected", Claim.total_claimed_amount)
        .else_(0)
    )
...
def get_expense_claim_types() -> list[dict]:
    ...
    return (
        frappe.qb.from_(ClaimType).select(ClaimType.name, ClaimType.description)
    ).run(as_dict=True)
...
def get_expense_approval_details(employee: str) -> dict:
    ...
    expense_approver_name = frappe.db.get_value(
        "User", expense_approver, "full_name", cache=True
    )
    ...
    if expense_approver and expense_approver not in [
        ...
        department_approvers.append(
            {"name": expense_approver, "full_name": expense_approver_name}
        )
...
def get_advance_account(company: str) -> str | None:
    return frappe.db.get_value(
        "Company", company, "default_employee_advance_account", cache=True
    )
...
def get_company_currencies() -> dict:
    ...
    return {
        company.name: (company.default_currency, company.symbol)
...
def get_doctype_fields(doctype: str) -> list[dict]:
    ...
    return [
        ...
        and field.fieldname != "amended_from"
    ]
