...
class TestShiftType(FrappeTestCase):
    ...
    def test_auto_update_last_sync_of_checkin_for_single_day_shift(self):
        ...
        employee = make_employee(
            "test_employee_checkin@example.com", company="_Test Company"
        )
        date = getdate()
        make_shift_assignment(shift_type.name, employee, date)

        # case 1: last sync updates from none to shift end after the shift end time
        frappe.flags.current_datetime = datetime.combine(
            getdate(), get_time("14:00:00")
        )
        update_last_sync_of_checkin()
        shift_type.reload()
        # last sync should be updated to 13:00:00
        self.assertEqual(
            shift_type.last_sync_of_checkin,
            datetime.combine(getdate(), get_time("13:01:00")),
        )

        # case 2: last sync doesn't update in the middle of the shift
        frappe.flags.current_datetime = add_days(
            datetime.combine(getdate(), get_time("12:00:00")), 1
        )
        update_last_sync_of_checkin()
        shift_type.reload()
        self.assertEqual(
    ...
    def test_auto_update_last_sync_of_checkin_for_shifts_spanning_two_days_due_to_buffer(
        self,
    ):
        shift_type = setup_shift_type(
        ...
        employee = make_employee(
            "test_employee_checkin3@example.com", company="_Test Company"
        )
        ...
        self.assertEqual(
            shift_type.last_sync_of_checkin,
            datetime.combine(getdate(), get_time("01:01:00")),
        )

        # case 2: last sync doesn't update in the middle of the shift
        frappe.flags.current_datetime = datetime.combine(
            getdate(), get_time("19:00:00")
        )
        update_last_sync_of_checkin()
        shift_type.reload()
        self.assertEqual(
    ...
    def test_auto_update_last_sync_of_checkin_for_two_day_shift(self):
        ...
        employee = make_employee(
            "test_employee_checkin2@example.com", company="_Test Company"
        )
        date = add_days(getdate(), -4)
        make_shift_assignment(shift_type.name, employee, date)
        ...
        self.assertEqual(
            shift_type.last_sync_of_checkin,
            datetime.combine(getdate(), get_time("07:01:00")),
        )

        # case 2: last sync doesn't update in the middle of the shift
        frappe.flags.current_datetime = add_days(
            datetime.combine(getdate(), get_time("01:00:00")), 1
        )
        update_last_sync_of_checkin()
        shift_type.reload()
        self.assertEqual(
    ...
    def test_auto_update_last_sync_of_checkin_when_when_job_runs_on_the_next_day(self):
        shift_type = setup_shift_type(
            shift_type="Test Long Shift", start_time="9:00:00", end_time="18:00:00"
        )
        ...
        frappe.flags.current_datetime = datetime.combine(
            add_days(getdate(), 1), get_time("00:01:00")
        )
        update_last_sync_of_checkin()
        ...
        self.assertEqual(
            shift_type.last_sync_of_checkin,
            datetime.combine(getdate(), get_time("23:31:00")),
        )
    ...
    def test_working_hours_threshold_for_absent_and_half_day_2(self):
        ...
        attendance = frappe.db.get_value(
            "Attendance", {"shift": shift_type.name}, "status"
        )
    ...
    def test_do_not_mark_absent_before_shift_actual_end_time(self):
        ...
        shift_1 = setup_shift_type(
            shift_type="Morning", start_time="07:00:00", end_time="12:30:00"
        )
        make_shift_assignment(
            shift_1.name, employee, add_days(yesterday, -1), yesterday
        )

        # shift 2
        shift_2 = setup_shift_type(
            shift_type="Afternoon", start_time="09:30:00", end_time="18:00:00"
        )
    ...
    def test_do_not_mark_absent_before_shift_actual_end_time_for_midnight_shift(self):
        ...
        shift_type.last_sync_of_checkin = datetime.combine(
            curr_date, get_time("00:30:00")
        )
        shift_type.save()
    ...
    def test_skip_absent_marking_for_inactive_employee(self):
        ...
        employee = make_employee(
            "test_inactive_employee@example.com", company="_Test Company"
        )
        date = getdate()
    ...
    def test_mark_attendance_for_default_shift_when_shift_assignment_is_not_overlapping(
        ...
        shift_1 = setup_shift_type(
            shift_type="Deafult Shift", start_time="08:00:00", end_time="12:00:00"
        )
        shift_2 = setup_shift_type(
            shift_type="Not Default Shift", start_time="10:00:00", end_time="18:00:00"
        )
        employee = make_employee(
        ...
        mark_attendance(
            employee, add_days(getdate(), -1), "Present", shift=shift_1.name
        )
    ...
    def test_validation_for_unlinked_logs_before_changing_important_shift_configuration(
        ...
        shift = setup_shift_type(
            shift_type="Test Shift", start_time="10:00:00", end_time="18:00:00"
        )
        employee = make_employee(
        ...
        self.assertEqual(
            frappe.get_value(
                "Shift Type", shift.name, "begin_check_in_before_shift_start_time"
            ),
            120,
        )
        ...
        shift_type = frappe.get_doc(
            {
                ...
                "determine_check_in_and_check_out": "Alternating entries as IN and OUT during the same shift",
                "working_hours_calculation_based_on": "First Check-in and Last Check-out",
                "begin_check_in_before_shift_start_time": args.begin_check_in_before_shift_start_time
                or 60,
                "allow_check_out_after_shift_end_time": args.allow_check_out_after_shift_end_time
                or 60,
                "process_attendance_after": add_days(date, -2),
                "last_sync_of_checkin": now_datetime() + timedelta(days=1),
                "mark_auto_attendance_on_holidays": args.mark_auto_attendance_on_holidays
                or False,
            }
        )
    else:
