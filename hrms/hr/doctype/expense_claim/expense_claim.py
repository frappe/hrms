...
class ExpenseClaim(AccountsController, PWANotificationsMixin):
                ...
                if (
                    ...
                    or (
                        ...
                        and (
                            ...
                            (
                                flt(self.grand_total, precision)
    ...
    def on_update_after_submit(self):
        if self.check_if_fields_updated(
            [], {"taxes": ("account_head",), "expenses": ()}
        ):
    ...
    def update_claimed_amount_in_employee_advance(self):
        for d in self.get("advances"):
            frappe.get_doc(
                "Employee Advance", d.employee_advance
            ).update_claimed_amount()
        ...
        for data in self.expenses:
            if not data.cost_center:
                frappe.throw(
                    ...
                    ).format(data.idx, frappe.bold(_("Cost Center")))
                )
...
def update_payment_for_expense_claim(doc, method=None):
    ...
    doctype_field = (
        "reference_type" if doc.doctype == "Journal Entry" else "reference_doctype"
    )
...
def get_allocation_amount(
    paid_amount=None, claimed_amount=None, return_amount=None, unclaimed_amount=None
):
    if unclaimed_amount is not None and return_amount is not None:
        ...
        frappe.throw(
            _("Invalid parameters provided. Please pass the required arguments.")
        )
