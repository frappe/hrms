...
class TestLeaveAllocation(FrappeTestCase):
    ...
    def test_earned_leaves_creation(self):
        ...
        self.assertEqual(
            get_leave_balance_on(
                self.employee.name, self.leave_type, frappe.flags.current_date
            ),
            2,
        )
    ...
    def test_pro_rated_allocation_via_scheduler(self):
        ...
        leaves_allocated = get_allocated_leaves(leave_policy_assignments[0])
        pro_rated_leave = round_earned_leaves(
            calculate_pro_rated_leaves(1, doj, start_date, get_last_day(start_date)),
            "0.5",
        )
        self.assertEqual(leaves_allocated, pro_rated_leave)
    ...
    def test_get_earned_leave_details_for_dashboard(self):
        ...
        first_sunday = get_first_sunday(
            self.holiday_list, for_date=frappe.flags.current_date
        )
        ...
        make_leave_application(
            self.employee.name, leave_date, leave_date, self.leave_type
        )
    ...
    def test_allocate_leaves_manually(self):
        ...
        leave_allocation.allocate_leaves_manually(
            1, add_days(frappe.flags.current_date, 1)
        )
        ...
        self.assertEqual(
            get_leave_balance_on(
                self.employee.name,
                self.leave_type,
                add_days(frappe.flags.current_date, 1),
            ),
            12,
        )

        self.assertRaises(
            frappe.ValidationError, leave_allocation.allocate_leaves_manually, 1
        )

    def test_quarterly_earned_leaves_allocated_on_last_day_in_the_middle_of_leave_period(
        self,
    ):
    ...
    def test_quarterly_earned_leaves_allocated_on_last_day_at_the_start_of_the_leave_period(
        self,
    ):
        frappe.flags.current_date = get_year_start(getdate())
    ...
    def test_quartertly_earned_leaves_allocated_on_first_day_at_the_start_of_leave_period(
        self,
    ):
        frappe.flags.current_date = get_year_start(getdate())
    ...
    def test_quarterly_leaves_allocated_pro_rated(self):
        ...
        employee = frappe.get_doc("Employee", "_T-Employee-00002")
        employee.date_of_joining = add_to_date(
            get_year_start(getdate()), months=1, days=10
        )
        employee.save()

        # make policy assignment on the same day
        frappe.flags.current_date = add_to_date(
            get_year_start(getdate()), months=1, days=10
        )
        assignment = make_policy_assignment(
            employee,
        ...
        frappe.flags.current_date = add_to_date(
            get_year_start(getdate()), months=3, days=-1
        )
        allocate_earned_leaves()
    ...
    def test_half_yearly_earned_leaves_allocated_on_last_day_at_the_start_of_leave_period(
        self,
    ):
        frappe.flags.current_date = get_year_start(getdate())
        employee = frappe.get_doc("Employee", "_T-Employee-00002")
    ...
    def test_half_yearly_earned_leaves_allocated_on_last_day_in_the_middle_of_leave_period(
        self,
    ):
        employee = frappe.get_doc("Employee", "_T-Employee-00002")
    ...
    def test_half_yearly_earned_leaves_allocated_on_first_day_at_the_start_of_leave_period(
        self,
    ):
        employee = frappe.get_doc("Employee", "_T-Employee-00002")
    ...
    def test_half_yearly_leaves_allocated_pro_rated(self):
        employee = frappe.get_doc("Employee", "_T-Employee-00002")
        employee.date_of_joining = add_to_date(
            get_year_start(getdate()), months=3, days=25
        )
        employee.save()
        ...
        frappe.flags.current_date = add_to_date(
            get_year_start(getdate()), months=6, days=-1
        )
        allocate_earned_leaves()
    ...
    def test_yearly_leaves_allocated_pro_rated(self):
        employee = frappe.get_doc("Employee", "_T-Employee-00002")
        employee.date_of_joining = add_to_date(
            get_year_start(getdate()), months=7, days=15
        )
        employee.save()
    ...
    def tearDown(self):
        ...
        frappe.db.set_value(
            "Employee", "_T-Employee-00002", "date_of_joining", self.original_doj
        )
        frappe.db.set_value("Leave Type", self.leave_type, "max_leaves_allowed", 0)
