...
class Interview(Document):
    ...
    def validate_designation(self):
        applicant_designation = frappe.db.get_value(
    ...
    def reschedule_interview(self, scheduled_on, from_time, to_time):
        if (
            ...
            and from_time == self.from_time
            and to_time == self.to_time
        ):
            frappe.msgprint(
...
def get_recipients(name, for_feedback=0):
    ...
    if for_feedback:
        ...
        recipients.append(
            frappe.db.get_value("Job Applicant", interview.job_applicant, "email_id")
        )
...
def send_interview_reminder():
    ...
    remind_before = (
        cstr(frappe.db.get_single_value("HR Settings", "remind_before")) or "01:00:00"
    )
