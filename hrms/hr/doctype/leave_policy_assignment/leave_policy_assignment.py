...
class LeavePolicyAssignment(Document):
    ...
    def get_leaves_for_passed_months(
        ...
        def _get_months_passed(current_date, from_date, consider_current_month):
            months_passed = 0
            if (
                current_date.year == from_date.year
                and current_date.month >= from_date.month
            ):
        ...
        months_passed = _get_months_passed(
            current_date, from_date, consider_current_month
        )

        if months_passed > 0:
            new_leaves_allocated = _calculate_leaves_for_passed_months(
                consider_current_month
            )
        else:
            new_leaves_allocated = 0
...
def show_assignment_submission_status(failed):
    ...
    assignment_list = [
        get_link_to_form("Leave Policy Assignment", entry) for entry in failed
    ]
    ...
    msg += (
        ...
        .format(
            "<a href='/app/List/Error Log?reference_doctype=Leave Policy Assignment'>{0}</a>"
        )
        .format(_("Error Log"))
    )
