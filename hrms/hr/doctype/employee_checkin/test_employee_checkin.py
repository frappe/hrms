...
from hrms.hr.doctype.shift_type.test_shift_type import (
...
class TestEmployeeCheckin(FrappeTestCase):
    ...
    def test_calculate_working_hours(self):
        ...
        self.assertEqual(
            working_hours, (5.0, logs_type_2[1].time, logs_type_2[-1].time)
        )
    ...
    def test_fetch_shift_for_assignment_with_end_date(self):
        ...
        shift2 = setup_shift_type(
            shift_type="Shift 2", start_time="12:30:00", end_time="16:30:00"
        )
    ...
    def test_shift_start_and_end_timings(self):
        ...
        self.assertEqual(
            log.shift_actual_start, datetime.combine(date, get_time("07:00:00"))
        )
        self.assertEqual(
            log.shift_actual_end, datetime.combine(date, get_time("13:00:00"))
        )

    def test_fetch_shift_based_on_default_shift(self):
        employee = make_employee(
            "test_default_shift@example.com", company="_Test Company"
        )
    ...
    def test_fetch_night_shift_for_assignment_without_end_date(self):
        ...
        for log in [log_in, log_out]:
            self.assertEqual(log.shift, shift_type.name)
            self.assertEqual(
                log.shift_start, datetime.combine(date, get_time("23:00:00"))
            )
            self.assertEqual(
                log.shift_end, datetime.combine(next_day, get_time("01:00:00"))
            )
            self.assertEqual(
                log.shift_actual_start, datetime.combine(date, get_time("22:00:00"))
            )
            self.assertEqual(
    ...
    def test_consecutive_shift_assignments_overlapping_within_grace_period(self):
        ...
        self.assertEqual(
            log.shift_actual_start, datetime.combine(date, get_time("12:00:00"))
        )
        ...
        self.assertEqual(log.shift_end, datetime.combine(date, get_time("12:00:00")))
        self.assertEqual(
            log.shift_actual_end, datetime.combine(date, get_time("12:00:00"))
        )
    ...
    def test_geofencing(self):
        ...
        # 15 - 19
        shift2 = setup_shift_type(
            shift_type="Consecutive Shift", start_time="15:00:00", end_time="19:00:00"
        )
        ...
        make_shift_assignment(
            shift1.name, employee, date, shift_location=location1.name
        )
        make_shift_assignment(
            shift2.name, employee, date, shift_location=location2.name
        )
    ...
    def test_bulk_fetch_shift_if_shift_settings_change_for_the_same_shift(self):
        ...
        log1 = make_checkin(emp1, timestamp)
        self.assertEqual(
            log1.shift_actual_start, datetime.combine(date, get_time("07:00:00"))
        )
        log2 = make_checkin(emp2, timestamp)
        self.assertEqual(
            log2.shift_actual_start, datetime.combine(date, get_time("07:00:00"))
        )
        ...
        log1.reload()
        self.assertEqual(
            log1.shift_actual_start, datetime.combine(date, get_time("06:00:00"))
        )
        log2.reload()
        self.assertEqual(
            log2.shift_actual_start, datetime.combine(date, get_time("06:00:00"))
        )
    ...
    def test_modifying_half_attendance_created_from_leave(self):
        ...
        emp = make_employee(
            "testhalfday@example.com", company="_Test Company", default_shift=shift.name
        )
        employee = frappe.get_doc("Employee", emp)
        # create attendance from leave
        leave_type = create_leave_type(
            leave_type_name="_Test Half Day", include_holidays=0
        )
    ...
    def test_half_day_attendance_when_employee_checkins_exists_and_attendance_is_full_day(
        self,
    ):
        shift = setup_shift_type(
            shift_type="_Test Half Day",
...
def make_n_checkins(employee, n, hours_to_reverse=1):
    logs = [
        make_checkin(
            employee, now_datetime() - timedelta(hours=hours_to_reverse, minutes=n + 1)
        )
    ]
    for i in range(n - 1):
        logs.append(
