...
const JOB_APPLICANT_STATUS_HEX_TO_INDICATOR = {
	"#bd3e0c": "orange",
	"#0070cc": "blue",
	"#b52a2a": "red",
	"#16794c": "green",
...
// Built synchronously from frappe.boot — no async fetch, no race condition.
const job_applicant_status_indicator_map = Object.fromEntries(
...
frappe.listview_settings["Job Applicant"] = {
	...
	get_indicator(doc) {
		const color = job_applicant_status_indicator_map[doc.status] || "gray";
...
document.addEventListener("click", function (e) {
	...
	const current_fraction = _kanban_rating_from_stars(rating_el, OUT_OF);
	...
	frappe.db.set_value("Job Applicant", docname, "applicant_rating", new_fraction).then(() => {
		_kanban_apply_stars(rating_el, new_fraction, OUT_OF);
	});
});

function _kanban_rating_from_stars(rating_el, out_of) {
	let value = 0;
...
function _kanban_apply_stars(rating_el, fraction, out_of) {
