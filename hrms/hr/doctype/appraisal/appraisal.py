            ...
            .where(
                ...
                & (
                    ...
                    | (
                        ...
                        | (
                            (self.end_date >= Appraisal.start_date)
                            & (self.end_date <= Appraisal.end_date)
                        )
                    )
                )
            )
        ...
        if self.rate_goals_manually:
            ...
            for entry in self.goals:
                if flt(entry.score) > 5:
                    frappe.throw(
                        _("Row {0}: Goal Score cannot be greater than 5").format(
                            entry.idx
                        )
                    )
