...
class EmployeeAdvance(Document):
    ...
    def validate_advance_account_type(self):
        account_type = frappe.db.get_value(
            "Account", self.advance_account, "account_type"
        )
    ...
    def set_status(self, update=False):
        ...
        total_amount = flt(
            flt(self.claimed_amount) + flt(self.return_amount), precision
        )
        status = None
        ...
        if self.docstatus == 0:
            ...
            if flt(self.claimed_amount) > 0 and flt(
                self.claimed_amount, precision
            ) == flt(self.paid_amount, precision):
                status = "Claimed"
            elif flt(self.return_amount) > 0 and flt(
                self.return_amount, precision
            ) == flt(self.paid_amount, precision):
                status = "Returned"
            elif (
            ...
            elif flt(self.paid_amount) > 0 and flt(
                self.advance_amount, precision
            ) == flt(self.paid_amount, precision):
                status = "Paid"
            else:
                status = "Unpaid"
    ...
    def set_total_advance_paid(self):
        ...
        if return_amount > 0 and return_amount > flt(
            paid_amount - self.claimed_amount, precision
        ):
    ...
    def update_claimed_amount(self):
        ...
        frappe.db.set_value(
            "Employee Advance", self.name, "claimed_amount", flt(claimed_amount)
        )
        self.reload()
        self.set_status(update=True)
    ...
    def check_linked_payment_entry(self):
        ...
        if frappe.db.get_single_value(
            "HR Settings", "unlink_payment_on_cancellation_of_employee_advance"
        ):
...
def make_bank_entry(dt, dn):
    ...
    je.multi_currency = (
        1 if advance_account_currency != payment_account.account_currency else 0
    )
...
def make_return_entry(
    ...
    je.multi_currency = (
        1 if advance_account_currency != bank_cash_account.account_currency else 0
    )
