...
class TestShiftAssignment(FrappeTestCase):
    ...
    def test_overlapping_for_ongoing_shift(self):
        ...
        non_overlapping_shift = make_shift_assignment(
            shift, employee, add_days(date, -1), add_days(date, -1)
        )
        self.assertEqual(non_overlapping_shift.docstatus, 1)

        overlapping_shift = make_shift_assignment(
            shift, employee, add_days(date, 2), do_not_submit=True
        )
    ...
    def test_multiple_shift_assignments_for_same_date(self):
        ...
        setup_shift_type(
            shift_type="Night Shift", start_time="19:00:00", end_time="23:00:00"
        )
        assignment = make_shift_assignment(
            "Night Shift", employee, date, do_not_submit=True
        )
    ...
    def test_overlapping_for_a_fixed_period_shift_and_ongoing_shift(self):
        employee = make_employee(
            "test_shift_assignment@example.com", company="_Test Company"
        )
        ...
        assignment = make_shift_assignment(
            "Shift 2", employee, date, do_not_submit=True
        )
        self.assertRaises(OverlappingShiftError, assignment.save)
    ...
    def test_overlap_for_shifts_on_same_day_with_overlapping_timeslots(self):
        ...
        setup_shift_type(
            shift_type="Shift 3", start_time="12:00:00", end_time="16:00:00"
        )
        make_shift_assignment("Shift 3", employee, date)

        # shift setup for 15-19
        setup_shift_type(
            shift_type="Shift 4", start_time="15:00:00", end_time="19:00:00"
        )
        assignment = make_shift_assignment(
            "Shift 4", employee, date, do_not_submit=True
        )
        self.assertRaises(OverlappingShiftError, assignment.save)
    ...
    def test_overlap_for_midnight_shifts(self):
        ...
        for i, pair in enumerate(overlapping_shifts):
            ...
            assignment = make_shift_assignment(
                s2.name, employee, date, do_not_submit=True
            )
        ...
        # no overlap
        shift_type = setup_shift_type(
            shift_type="Shift 3", start_time="01:00:00", end_time="05:00:00"
        )
        assignment = make_shift_assignment(shift_type.name, employee, date)

        # overlap
        shift_type = setup_shift_type(
            shift_type="Shift 4", start_time="21:00:00", end_time="02:00:00"
        )
        assignment = make_shift_assignment(
            shift_type.name, employee, date, do_not_submit=True
        )
        self.assertRaises(OverlappingShiftError, assignment.save)

    def test_calendar(self):
        employee1 = make_employee(
            "test_shift_assignment1@example.com", company="_Test Company"
        )
        employee2 = make_employee(
            "test_shift_assignment2@example.com", company="_Test Company"
        )
        employee3 = make_employee(
            "test_shift_assignment3@example.com", company="_Test Company"
        )
        ...
        make_shift_assignment(
            shift_type.name, employee2, date
        )  # excluded due to employee filter
        make_shift_assignment(
            shift_type.name, employee3, add_days(date, -3), add_days(date, -2)
        )  # excluded
        shift2 = make_shift_assignment(
            shift_type.name, employee3, add_days(date, -1), date
        )  # 2 days
        ...
        make_shift_assignment(
            shift_type.name, employee3, add_days(date, 31)
        )  # excluded
        ...
        for shift in events:
            self.assertIn(
                shift["name"], [shift1.name, shift2.name, shift3.name, shift4.name]
            )
    ...
    def test_calendar_for_night_shift(self):
        ...
        self.assertEqual(
            events[0]["end_date"], get_datetime(f"{add_days(date, 1)} 02:00:00")
        )
    ...
    def test_shift_details_on_consecutive_days_with_overlapping_timings(self):
        ...
        # shift 1
        shift_type = setup_shift_type(
            shift_type="Morning", start_time="07:00:00", end_time="12:00:00"
        )
        make_shift_assignment(
            shift_type.name, employee, add_days(yesterday, -1), yesterday
        )

        # shift 2
        shift_type = setup_shift_type(
            shift_type="Afternoon", start_time="09:30:00", end_time="14:00:00"
        )
        ...
        self.assertTrue(
            checkin.shift_type.name == checkout.shift_type.name == "Morning"
        )
