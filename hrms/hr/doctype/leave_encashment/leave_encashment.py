...
class LeaveEncashment(AccountsController):
    ...
    def set_actual_encashable_days(self):
        ...
        if encashment_settings.non_encashable_leaves:
            actual_encashable_days = (
                self.leave_balance - encashment_settings.non_encashable_leaves
            )
            self.actual_encashable_days = (
                actual_encashable_days if actual_encashable_days > 0 else 0
            )
            frappe.msgprint(
    ...
    def create_additional_salary(self):
        ...
        if not earning_component:
            frappe.throw(
                _("Please set Earning Component for Leave type: {0}.").format(
                    self.leave_type
                )
            )
    ...
    def set_total_advance_paid(self):
        ...
        if flt(paid_amount) > self.encashment_amount:
            frappe.throw(
                _("Row {0}# Paid Amount cannot be greater than Encashment amount")
            )
