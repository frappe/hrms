...
class TestLeaveEncashment(FrappeTestCase):
    def setUp(self):
        ...
        self.holiday_list = make_holiday_list(
            "_Test Leave Encashment", year_start, year_end
        )

        # create the leave policy
        leave_policy = create_leave_policy(
            leave_type=self.leave_type, annual_allocation=10
        )
        leave_policy.submit()
    ...
    def test_non_encashable_leaves_setting(self):
        ...
        first_sunday = get_first_sunday(
            self.holiday_list, for_date=self.leave_period.from_date
        )
    ...
    def test_creation_of_leave_ledger_entry_on_submit(self):
        ...
        self.assertEqual(
            leave_ledger_entry[0].leaves, leave_encashment.encashment_days * -1
        )
        ...
        self.assertFalse(
            frappe.db.exists(
                "Leave Ledger Entry", {"transaction_name": leave_encashment.name}
            )
        )
    ...
    def test_unused_leaves_after_leave_encashment_for_carry_forwarding_leave_type(self):
        employee = make_employee(
            "test_employee2_encashment@example.com", company="_Test Company"
        )
        ...
        self.assertEqual(
            frappe.get_value("Leave Type", self.leave_type, "is_carry_forward"), 1
        )
        leave_ledger_entry = frappe.get_all(
            "Leave Ledger Entry",
            fields=["leaves"],
    ...
    def test_leave_expiry_after_leave_encashment_for_non_carry_forwarding_leave_type(
        ...
        employee = make_employee(
            "test_employee3_encashment@example.com", company="_Test Company"
        )
        ...
        self.assertEqual(
            frappe.get_value("Leave Type", self.leave_type, "is_carry_forward"), 0
        )
        leave_ledger_entry = frappe.get_all(
            "Leave Ledger Entry",
            fields="*",
        ...
        self.assertEqual(
            leave_ledger_entry[1].leaves, leave_encashment.encashment_days * 1
        )
    ...
    def get_encashment_created_after_leave_period(
        self, employee, is_carry_forward, encashment_days
    ):
        ...
        leave_policy = frappe.get_value(
            "Leave Policy", {"title": "Test Leave Policy"}, "name"
        )
        data = {
            "assignment_based_on": "Leave Period",
    ...
    def test_status_of_leave_encashment_after_payment_via_salary_slip(self):
        ...
        ss = make_employee_salary_slip(
            self.employee, "Monthly", salary_structure=salary_structure.name
        )
    ...
    def test_status_of_leave_encashment_after_payment_via_payment_entry_and_fnf(self):
        ...
        pe = get_payment_entry_for_employee(
            leave_encashment.doctype, leave_encashment.name
        )
        pe.reference_no = "1"
        ...
        jv.accounts[1].account = (
            frappe.get_cached_value("Company", "_Test Company", "default_bank_account")
            or "_Test Bank - _TC"
        )
        jv.cheque_no = "123456"
...
def create_leave_encashment(**args):
    ...
    if leave_encashment.pay_via_payment_entry:
        leave_encashment.payable_account = (
            args.payable_account
            or frappe.get_cached_value(
                "Company", "_Test Company", "default_payable_account"
            )
        )
        leave_encashment.expense_account = (
            args.expense_account or "Administrative Expenses - _TC"
        )
