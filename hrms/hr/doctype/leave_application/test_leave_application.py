...
class TestLeaveApplication(HRMSTestSuite):
    ...
    def make_leave_applications(cls):
        ...
        for x in records:
            if not frappe.db.exists(
                "Leave Application",
                {"employee": x.get("employee"), "from_date": x.get("from_date")},
            ):
    ...
    def test_validate_application_across_allocations(self):
        ...
        first_sunday = get_first_sunday(
            self.holiday_list, for_date=get_year_start(date)
        )
    ...
    def test_insufficient_leave_balance_validation(self):
        ...
        frappe.db.set_value(
            "Leave Type", "Test Leave Validation", "allow_negative", True
        )
        make_leave_application(
            employee.name,
    ...
    def test_separate_leave_ledger_entry_for_boundary_applications(self):
        ...
        make_allocation_record(
            leave_type=leave_type.name, from_date=year_start, to_date=year_end
        )
    ...
    def test_attendance_for_include_holidays(self):
        ...
        self.assertEqual(
            frappe.db.count(
                "Attendance", {"leave_application": leave_application.name}
            ),
            4,
        )
    ...
    def test_attendance_update_for_exclude_holidays(self):
        ...
        frappe.delete_doc_if_exists(
            "Leave Type", "Test Do Not Include Holidays", force=1
        )
        leave_type = frappe.get_doc(
            {
        ...
        self.assertEqual(
            frappe.db.get_value("Attendance", attendance.name, "status"), "On Leave"
        )
    ...
    def test_optional_leave(self):
        ...
        self.assertEqual(
            get_leave_balance_on(employee.name, leave_type, optional_leave_date), 9
        )
    ...
    def test_current_leave_on_submit(self):
        ...
        if not frappe.db.exists("Leave Type", leave_type):
            frappe.get_doc(
                dict(leave_type_name=leave_type, doctype="Leave Type")
            ).insert()
    ...
    def test_creation_of_leave_ledger_entry_on_submit(self):
        ...
        self.assertFalse(
            frappe.db.exists(
                "Leave Ledger Entry", {"transaction_name": leave_application.name}
            )
        )
    ...
    def test_leave_approver_perms(self):
        ...
        application.insert()
        self.assertTrue(
            application.name in frappe.share.get_shared("Leave Application", user)
        )
        ...
        application.save()
        self.assertTrue(
            application.name not in frappe.share.get_shared("Leave Application", user)
        )
    ...
    def test_self_leave_approval_allowed(self):
        ...
        employee = frappe.get_doc(
            "Employee",
            make_employee(
                "test_self_leave_approval@example.com",
                "_Test Company",
                leave_approver="test@example.com",
            ),
        )
    ...
    def test_leave_details_with_expired_cf_leaves(self):
        ...
        self.assertEqual(
            leave_details["leave_allocation"][leave_type.name], expected_data
        )
    ...
    def test_modifying_attendance_from_absent_to_half_day(self):
        ...
        attendance_name = mark_attendance(
            employee=employee.name, attendance_date=nowdate(), status="Absent"
        )
    ...
    def test_half_day_status_for_two_half_leaves(self):
        ...
        half_day_status_after_first_application = frappe.get_value(
            ...
            filters={
                "attendance_date": nowdate(),
                "leave_application": first_leave_application.name,
            },
        ...
        half_day_status_after_second_application = frappe.get_value(
            ...
            filters={
                "attendance_date": nowdate(),
                "leave_application": second_leave_application.name,
            },
            fieldname="half_day_status",
        )
...
def allocate_leaves(
    employee, leave_period, leave_type, new_leaves_allocated, eligible_leaves=0
):
