	...
	def get_consecutive_leave_details(self) -> dict:
		...
		def _get_first_from_date(reference_date):
			...
			application = frappe.db.get_value(
				"Leave Application",
				{"employee": self.employee, "leave_type": self.leave_type, "to_date": prev_date, "docstatus": 1, "status": ["in",["Open","Approved"]]},
				["name", "from_date"],
				as_dict=True,
			)
			if application:
		...
		def _get_last_to_date(reference_date):
			...
			application = frappe.db.get_value(
				"Leave Application",
				{"employee": self.employee, "leave_type": self.leave_type, "from_date": next_date, "docstatus": 1, "status": ["in",["Open","Approved"]]},
				["name", "to_date"],
				as_dict=True,
			)
			if application:
