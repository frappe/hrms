...
def get_formatted_value(value, fieldtype):
    ...
    if fieldtype == "Date":
        ...
        decimal_str, comma_str, _number_format_precision = get_number_format_info(
            number_format
        )
...
def validate_duplicate_exemption_for_payroll_period(
    doctype, docname, payroll_period, employee
):
...
def validate_tax_declaration(declarations):
    ...
    for d in declarations:
        if d.exemption_sub_category in subcategories:
            frappe.throw(
                _("More than one selection for {0} not allowed").format(
...
def get_total_exemption_amount(declarations):
    ...
    for d in declarations:
        ...
        if (
            category_max_amount
            and exemptions.get(d.exemption_category).total_exemption_amount
            > category_max_amount
        ):
            exemptions.get(d.exemption_category).total_exemption_amount = (
                category_max_amount
            )
...
def generate_leave_encashment():
    ...
    if frappe.db.get_single_value("HR Settings", "auto_leave_encashment"):
        leave_type = frappe.get_all(
            "Leave Type", filters={"allow_encashment": 1}, fields=["name"]
        )
...
def update_previous_leave_allocation(
    ...
    annual_allocation = flt(
        annual_allocation, allocation.precision("total_leaves_allocated")
    )
    ...
    if (
        ...
        and e_leave_type.max_leaves_allowed > 0
    ):
        new_allocation = e_leave_type.max_leaves_allowed
...
def get_holidays_for_employee(
    ...
    holiday_list = get_holiday_list_for_employee(
        employee, raise_exception=raise_exception
    )
...
def get_previous_claimed_amount(
    employee, payroll_period, non_pro_rata=False, component=False
):
...
def share_doc_with_approver(doc, user):
    ...
    if not frappe.has_permission(doc=doc, ptype="submit", user=user):
        ...
        frappe.msgprint(
            _("Shared with the user {0} with 'submit' permisions").format(
                user, alert=True
            )
        )
...
def get_ec_matching_query(
    ...
    if common_filters:
        ref_rank = (
            frappe.qb.terms.Case().when(ec.employee == common_filters.party, 1).else_(0)
            + 1
        )
