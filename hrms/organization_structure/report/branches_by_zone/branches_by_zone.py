...
def get_data(filters):
	...
	branches = frappe.get_all(
		"Organization Unit",
		filters=conditions,
		fields=["zone", "region", "status"],
	)

	summary = {}
	for branch in branches:
		key = (branch.zone or "", branch.region or "")
	...
	data = []
	for (zone, region), row in sorted(summary.items()):
		row.update({"zone": zone or None, "region": region or None})
		data.append(row)
