...
class SalarySlip(TransactionBase):
    ...
    def actual_start_date(self):
        if not hasattr(self, "__actual_start_date"):
            ...
            if self.joining_date and getdate(
                self.start_date
            ) < self.joining_date <= getdate(self.end_date):
    ...
    def actual_end_date(self):
        if not hasattr(self, "__actual_end_date"):
            ...
            if self.relieving_date and getdate(
                self.start_date
            ) <= self.relieving_date < getdate(self.end_date):
    ...
    def check_salary_withholding(self):
        withholding = get_salary_withholdings(
    ...
    def set_net_total_in_words(self):
        ...
        total = (
            self.net_pay if self.is_rounding_total_disabled() else self.rounded_total
        )
        base_total = (
            self.base_net_pay
    ...
    def is_rounding_total_disabled(self):
        return cint(
            frappe.db.get_single_value("Payroll Settings", "disable_rounded_total")
        )
    ...
    def check_existing(self):
        if not self.salary_slip_based_on_timesheet:
            ...
            for data in self.timesheets:
                if (
                    frappe.db.get_value("Timesheet", data.time_sheet, "status")
                    == "Payrolled"
                ):
                    frappe.throw(
                        _(
    ...
    def get_payment_days(self, include_holidays_in_total_working_days):
        ...
        if self.relieving_date:
            ...
            if (
                self.relieving_date < getdate(self.start_date)
                and employee_status != "Left"
            ):
                frappe.throw(
    ...
    def calculate_lwp_or_ppl_based_on_leave_application(
        ...
        for d in working_days_list:
            ...
            fraction_of_daily_salary_per_leave = flt(
                leave.fraction_of_daily_salary_per_leave
            )
            ...
            if cint(leave.is_ppl):
                equivalent_lwp_count *= (
                    (1 - fraction_of_daily_salary_per_leave)
    ...
    def compute_non_taxable_earnings(self):
        ...
        future_period_non_taxable_earnings = (
            self.get_future_period_non_taxable_earnings()
        )
    ...
    def get_future_period_non_taxable_earnings(self):
        ...
        return future_period_non_taxable_earnings * (
            ceil(self.remaining_sub_periods) - 1
        )
    ...
    def add_structure_component(self, struct_row, component_type):
        if (
            ...
            == self._salary_structure_doc.salary_component
        ):
            return
    ...
    def get_component_abbr_map(self):
        ...
        return frappe.cache().get_value(
            SALARY_COMPONENT_VALUES, generator=_fetch_component_values
        )
    ...
    def add_employee_benefits(self):
        ...
        self.benefit_details_parent, self.benefit_details_doctype = (
        ...
        if employee_benefits:
            employee_benefits = self.get_current_period_employee_benefit_amounts(
                employee_benefits
            )
    ...
    def add_current_period_employee_benefits(self, employee_benefits: dict):
        ...
        for benefit in employee_benefits:
            ...
            remarks = (
                "Pro rata flexible benefit accrual"
                if benefit.is_accrual
                else "Flexible benefit payout"
            )
    ...
    def get_current_period_employee_benefit_amounts(
        ...
        if self.payroll_period and getdate(self.payroll_period.end_date) <= getdate(
            self.end_date
        ):
        ...
        for benefit in employee_benefits:
            current_period_benefit = (
                benefit.yearly_amount / total_sub_periods if total_sub_periods else 0
            )
            if benefit.depends_on_payment_days:
                current_period_benefit = (
                    flt(current_period_benefit)
            ...
            current_period_benefit, is_accrual = (
                self._get_benefit_amount_and_transaction_type(
                    benefit,
                    current_period_benefit,
                    total_accrued,
                    total_paid,
                    is_last_payroll_cycle,
                )
            )
    ...
    def _get_benefit_ledger_entries(self, employee_benefits):
        ...
        ledger_entries = frappe.get_all(
            ...
            filters={
                ...
                "salary_component": [
                    "in",
                    [benefit.salary_component for benefit in employee_benefits],
                ],
                "payroll_period": self.payroll_period.name,
            },
        ...
        for entry in ledger_entries:
            benefit_ledger_map[entry["salary_component"]][
    ...
    def _get_final_period_benefit_payout(
        ...
        claimed_amount = (
            sum(row.amount for row in benefit_claims) if benefit_claims else 0
        )
        ...
        if is_last_payroll_cycle:  # On last payroll cycle, pay out all accrued benefits
            current_period_benefit = max(
                total_accrued + current_period_benefit - total_paid, 0
            )
            is_accrual = 0
    ...
    def add_additional_salary_components(self, component_type):
        ...
        for additional_salary in additional_salaries:
            ...
            if component_type == "earnings" and hasattr(
                self, "benefit_ledger_components"
            ):
                if (
    ...
    def add_tax_components(self):
        ...
        if self.handle_additional_salary_tax_component():
            self._component_based_variable_tax.setdefault(
                self.additional_salary_component, {}
            )
        ...
        for tax_component in tax_components:
            ...
            if self._component_based_variable_tax[tax_component]:
                tax_amount = self._component_based_variable_tax[tax_component][
                    "current_tax_amount"
                ]
    ...
    def handle_additional_salary_tax_component(self) -> bool:
        component = next(
            (
                d
                for d in self.get("deductions")
                if d.variable_based_on_taxable_salary and d.additional_salary
            ),
            None,
        )
    ...
    def get_income_tax_slabs(self):
        ...
        if getdate(income_tax_slab_doc.effective_from) > getdate(
            self.payroll_period.start_date
        ):
            frappe.throw(
                _(
    ...
    def get_future_recurring_additional_amount(
        self, additional_salary, monthly_additional_amount
    ):
    ...
    def get_amount_based_on_payment_days(self, row):
        ...
        if not row.additional_salary and not row.default_amount:
            ...
            and (
                ...
                or (
                    self.relieving_date and getdate(self.end_date) > self.relieving_date
                )
            )
        ):
    ...
    def email_salary_slip(self):
        receiver = frappe.db.get_value(
            "Employee", self.employee, "prefered_email", cache=True
        )
        ...
        if payroll_settings.email_template:
            email_template = frappe.get_doc(
                "Email Template", payroll_settings.email_template
            )
        ...
        if self.salary_slip_based_on_timesheet == 1:
            ...
            if hasattr(self, "deductions"):
                for deduction in self.deductions:
                    self.total_deduction += flt(
                        deduction.amount, deduction.precision("amount")
                    )
            self.net_pay = (
                flt(self.gross_pay)
    ...
    def calculate_total_for_salary_slip_based_on_timesheet(self):
        ...
        if self.earnings:
            for i, earning in enumerate(self.earnings):
                ...
                self.gross_pay += flt(
                    self.earnings[i].amount, earning.precision("amount")
                )
    ...
    def compute_year_to_date(self):
        ...
        gross_year_to_date = (
            flt(salary_slip_sum[0].gross_sum) if salary_slip_sum else 0.0
        )
    ...
    def add_leave_balances(self):
        ...
        if frappe.db.get_single_value(
            "Payroll Settings", "show_leave_balances_in_salary_slip"
        ):
            ...
            for leave_type, leave_values in leave_details["leave_allocation"].items():
                self.append(
                    "leave_details",
                    {
                        ...
                        "pending_leaves": flt(
                            leave_values.get("leaves_pending_approval")
                        ),
...
def get_lwp_or_ppl_for_date_range(employee, start_date, end_date):
    ...
    leaves = (
        ...
        .where(
            ...
            & (
                (LeaveApplication.salary_slip.isnull())
                | (LeaveApplication.salary_slip == "")
            )
            & (
...
def _safe_eval(
    code: str, eval_globals: dict | None = None, eval_locals: dict | None = None
):
...
def enqueue_email_salary_slips(names) -> None:
    ...
    frappe.enqueue(
        "hrms.payroll.doctype.salary_slip.salary_slip.email_salary_slips", names=names
    )
    frappe.msgprint(
        _(
