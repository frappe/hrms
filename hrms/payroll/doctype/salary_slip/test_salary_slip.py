...
class TestSalarySlip(FrappeTestCase):
    ...
    def tearDown(self):
        frappe.db.set_single_value(
            "Payroll Settings", "include_holidays_in_total_working_days", 0
        )
    ...
    def test_leave_details(self):
        ...
        make_leave_application(
            emp_id, first_sunday, add_days(first_sunday, 3), "_Test Leave Type"
        )
        next_month = add_months(nowdate(), 1)
        make_leave_application(
            emp_id, next_month, add_days(next_month, 3), "_Test Leave Type"
        )
    ...
    def test_employee_status_inactive(self):
        ...
        salary_slip = make_salary_slip(
            salary_structure.name, employee=employee_doc.name
        )
    ...
    def test_payment_days_based_on_attendance(self):
        ...
        frappe.db.set_value(
            "Employee", emp_id, {"relieving_date": None, "status": "Active"}
        )
        ...
        mark_attendance(
            emp_id, first_sunday, "Absent", ignore_validate=True
        )  # invalid lwp
        mark_attendance(
    ...
    def test_payment_days_for_mid_joinee_including_holidays(self):
        no_of_days = get_no_of_days()
        month_start_date, month_end_date = get_first_day(nowdate()), get_last_day(
            nowdate()
        )

        new_emp_id = make_employee("test_payment_days_based_on_joining_date@salary.com")
        joining_date, relieving_date = add_days(month_start_date, 3), add_days(
            month_end_date, -5
        )
    ...
    def test_payment_days_calculation_for_lwp_on_month_boundaries(self):
        ...
        make_leave_application(
            emp_id, "2024-06-28", "2024-07-03", "Leave Without Pay"
        )  # 3 days in July
        make_leave_application(
            emp_id, "2024-07-10", "2024-07-13", "Leave Without Pay"
        )  # 4 days in July
        make_leave_application(
            emp_id, "2024-07-28", "2024-08-05", "Leave Without Pay"
        )  # 3 days in July
    ...
    def test_payment_days_in_salary_slip_based_on_timesheet(self):
        ...
        frappe.db.set_value(
            "Employee", emp, {"relieving_date": None, "status": "Active"}
        )
        ...
        for row in salary_slip.earnings:
            if row.salary_component == "Basic Salary":
                ...
                break
        expected_amount = flt(
            (50000 * salary_slip.payment_days / salary_slip.total_working_days),
            precision,
        )
    ...
    def test_consider_marked_attendance_on_holidays(self):
        ...
        frappe.db.set_single_value(
            "Payroll Settings", "consider_marked_attendance_on_holidays", 0
        )
        ss.save()
        self.assertEqual(ss.total_working_days, no_of_days[0])
    ...
    def test_consider_marked_attendance_on_holidays_with_unmarked_attendance(self):
        ...
        frappe.db.delete(
            "Attendance",
            {"employee": emp_id, "attendance_date": add_days(first_sunday, 1)},
        )
    ...
    def test_consider_marked_attendance_on_holidays_with_half_day_on_holiday(self):
        ...
        frappe.db.set_single_value(
            "Payroll Settings", "consider_marked_attendance_on_holidays", 1
        )
        ss.save()
        self.assertEqual(ss.total_working_days, no_of_days[0])
    ...
    def test_email_salary_slip(self):
        ...
        emp_id = make_employee(
            "test_email_salary_slip@salary.com", company="_Test Company"
        )
    ...
    def test_component_wise_year_to_date_computation(self):
        ...
        # clear salary slip for this employee
        frappe.db.sql(
            "DELETE FROM `tabSalary Slip` where employee_name = '%s'" % employee_name
        )
    ...
    def test_statistical_component_based_on_payment_days(self):
        ...
        self.assertEqual(
            amount,
            flt(
                flt((1000 * ss.payment_days / ss.total_working_days), precision) * 0.5,
                precision,
            ),
        )
    ...
    def test_salary_slip_generation_against_opening_entries_in_ssa(self):
        ...
        effective_date = frappe.db.get_value(
            "Income Tax Slab", tax_slab, "effective_from"
        )

        if effective_date != "2022-04-01":
            frappe.db.set_value(
                "Income Tax Slab", tax_slab, "effective_from", "2022-04-01"
            )
    ...
    def test_income_tax_breakup_fields(self):
        ...
        self.assertEqual(
            flt(salary_slip.income_tax_deducted_till_date, 2), monthly_tax_amount
        )
        self.assertEqual(
            flt(salary_slip.current_month_income_tax, 2), monthly_tax_amount
        )
    ...
    def test_income_tax_breakup_when_tax_added_via_additional_salary(self):
        ...
        salary_structure_name = (
            "Test Salary Structure to test Income Tax Breakup added via addnl salary"
        )
        if not frappe.db.exists("Salary Structure", salary_structure_name):
            salary_structure_doc = make_salary_structure(
                salary_structure_name,
        ...
        create_additional_salary_for_income_tax(
            emp, payroll_period, company="_Test Company"
        )
        ...
        self.assertEqual(
            flt(salary_slip.future_income_tax_deductions, 2), 124843.25
        )  # as 136843.25 - 12000
        self.assertEqual(flt(salary_slip.total_income_tax, 2), 136843.25)
    ...
    def test_lwp_calculation_based_on_relieving_date(self):
        ...
        make_leave_application(
            emp_id, leave_start_date, leave_end_date, "Leave Without Pay"
        )
        ...
        days_between_start_and_relieving = (
            date_diff(relieving_date, month_start_date) + 1
        )

        self.assertEqual(ss.leave_without_pay, 0)

        self.assertEqual(
            ss.payment_days, (days_between_start_and_relieving - len(holidays))
        )
    ...
    def test_variable_tax_component(self):
        ...
        self.assertNotIn(
            "_Test TDS", [com.salary_component for com in salary_slip.deductions]
        )
    ...
    def test_opening_balances_excluded_from_tax_calculation(self):
        ...
        # Create salary slip
        salary_slip = make_salary_slip(
            salary_structure.name, employee=emp, posting_date="2023-04-01"
        )
        ...
        self.assertEqual(
            salary_slip.income_tax_deducted_till_date,
            salary_slip.current_month_income_tax,
        )
    ...
    def test_tax_payable_with_tax_relief_and_marginal_relief_limits(self):
        ...
        tax_relief_limit, marginal_relief_limit = frappe.db.get_value(
            "Income Tax Slab",
            {"currency": "INR"},
            ["tax_relief_limit", "marginal_relief_limit"],
        )
...
class TestSalarySlipSafeEval(FrappeTestCase):
    def test_safe_eval_for_salary_slip(self):
        ...
        user.enabled = 1
        self.assertTrue(
            _safe_eval("user_type == 'System User'", eval_locals=user.as_dict())
        )
        self.assertEqual(
            "System User Test",
            _safe_eval("user_type + ' Test'", eval_locals=user.as_dict()),
        )
...
def get_no_of_days():
    no_of_days_in_month = calendar.monthrange(
        getdate(nowdate()).year, getdate(nowdate()).month
    )
...
def make_salary_component(salary_components, test_tax, company_list=None):
    for salary_component in salary_components:
        if frappe.db.exists("Salary Component", salary_component["salary_component"]):
            frappe.delete_doc(
                "Salary Component", salary_component["salary_component"], force=True
            )
...
def create_tax_slab(
    ...
    income_tax_slab_name = frappe.db.get_value(
        "Income Tax Slab", {"currency": currency, "docstatus": 1}
    )
    ...
    if not income_tax_slab_name:
        ...
        income_tax_slab.append(
            "other_taxes_and_charges", {"description": "cess", "percent": 4}
        )
...
def setup_test():
    ...
    frappe.db.set_single_value(
        "HR Settings", "leave_status_notification_template", None
    )
    frappe.db.set_single_value(
        "HR Settings", "leave_approval_notification_template", None
    )
...
def make_payroll_period():
    ...
    for company in company_based_payroll_period:
        ...
        if not payroll_period:
            create_payroll_period(
                company=company, name=company_based_payroll_period[company]
            )
...
def make_salary_structure_for_timesheet(employee, company=None):
    ...
    if not frappe.db.get_value(
        ...
        frappe.db.set_value(
            "Employee", employee, "date_of_joining", add_months(nowdate(), -5)
        )
...
def create_additional_salary_for_non_taxable_component(
...
def make_salary_structure_for_statistical_component(company):
    earnings = [
        ...
        {
            "salary_component": "HRA Component",
            "abbr": "HRAC",
            "amount": 3000,
            "type": "Earning",
        },
