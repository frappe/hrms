...
class SalaryComponent(Document):
    def before_validate(self):
        self._condition, self.condition = self.condition, sanitize_expression(
            self.condition
        )
    ...
    def validate_abbr(self):
        if not self.salary_component_abbr:
            self.salary_component_abbr = "".join(
                [c[0] for c in self.salary_component.split()]
            ).upper()
    ...
    def validate_accounts(self):
        if not (
            self.statistical_component
            or (self.accounts and all(d.account for d in self.accounts))
        ):
            frappe.msgprint(
                title=_("Warning"),
    ...
    def valide_arrear_component(self):
        if self.variable_based_on_taxable_salary and self.arrear_component:
            frappe.throw(
                _(
                    "Arrear Component cannot be set for Salary Components based on taxable salary."
                ),
    ...
    def get_structures_to_be_updated(self):
        ...
        return (
            ...
            .where(
                (SalaryDetail.salary_component == self.name)
                & (SalaryStructure.docstatus != 2)
            )
            .run(pluck=True)
        )
    ...
    def update_salary_structures(self, field, value, structures=None):
        ...
        for structure in structures:
            ...
            if is_formula_related:
                ...
                salary_detail_row.set(
                    "amount_based_on_formula", self.amount_based_on_formula
                )
