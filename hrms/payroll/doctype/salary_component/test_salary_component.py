...
class TestSalaryComponent(FrappeTestCase):
	def test_update_salary_structures(self):
		...
		ss1_detail = [d for d in salary_structure1.earnings if d.salary_component == "Special Allowance"][0]
		self.assertEqual(ss1_detail.condition, "H < 10000")
		self.assertEqual(ss1_detail.formula, "BS*.5")

		ss2_detail = [d for d in salary_structure2.earnings if d.salary_component == "Special Allowance"][0]
		self.assertEqual(ss2_detail.condition, "H < 10000")
		self.assertEqual(ss2_detail.formula, "BS*.5")

		ss3_detail = [d for d in salary_structure3.earnings if d.salary_component == "Special Allowance"][0]
