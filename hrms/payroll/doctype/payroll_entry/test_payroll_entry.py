...
from hrms.payroll.doctype.salary_component.test_salary_component import (
...
class TestPayrollEntry(FrappeTestCase):
    def setUp(self):
        ...
        frappe.db.set_single_value(
            "Payroll Settings", "email_salary_slip_to_employee", 0
        )
        frappe.db.set_value(
            "Account", "Employee Advances - _TC", "account_type", "Receivable"
        )
    ...
    def test_employee_cost_center_breakup(self):
        ...
        department = create_department("Cost Center Test")
        employee = make_employee(
            "test_emp1@example.com", department=department, company=COMPANY
        )
        ...
        old_assignment.append(
            "payroll_cost_centers", {"cost_center": "Main - _TC", "percentage": 100}
        )
        ...
        cost_centers = pe.get_payroll_cost_centers_for_employee(
            employee, "_Test Salary Structure 2"
        )
        self.assertEqual(cost_centers, COST_CENTERS)

    def test_get_end_date(self):
        self.assertEqual(
            get_end_date("2017-01-01", "monthly"), {"end_date": "2017-01-31"}
        )
        self.assertEqual(
        ...
        self.assertEqual(
            get_end_date("2017-02-15", "monthly"), {"end_date": "2017-03-14"}
        )
        self.assertEqual(
            get_end_date("2017-02-15", "daily"), {"end_date": "2017-02-15"}
        )
    ...
    def test_payroll_entry_cancellation_against_cancelled_journal_entry(self):
        company_doc = frappe.get_doc("Company", "_Test Company")
        employee = make_employee(
            "test_pe_cancellation@payroll.com", company=company_doc.name
        )
    ...
    def test_advance_deduction_in_accrual_journal_entry(self):
        ...
        advance = make_employee_advance(
            employee, {"repay_unclaimed_amount_from_salary": 1}
        )
        ...
        component = create_salary_component(
            "Advance Salary - Deduction", **{"type": "Deduction"}
        )
        component.append(
            "accounts",
    ...
    def test_skip_bank_entry_for_employees_with_zero_amount(self):
        company_doc = frappe.get_doc("Company", "_Test Company")
        employee1 = make_employee(
            "test_employee11@payroll.com", company=company_doc.name
        )
        employee2 = make_employee(
            "test_employee12@payroll.com", company=company_doc.name
        )
    ...
    def run_test_for_loan_repayment_from_salary(self):
        ...
        initial_gross_pay = flt(salary_slip.gross_pay) - flt(
    ...
    def test_component_exclusion_from_accounting_entries(self):
        company = frappe.get_doc("Company", "_Test Company")
        employee = make_employee(
            "exclude_component_test@payroll.com", company=company.name
        )
        ...
        self.assertTrue(
            any(row.salary_component == esi.name for row in salary_slip.deductions)
        )
...
def create_assignments_with_cost_centers(employee1, employee2):
    ...
    ss = setup_salary_structure(
        employee2, company, salary_structure="_Test Salary Structure 2"
    )
...
def setup_lending():
    ...
    applicant = make_employee(
        "test_employee@loan.com", company="_Test Company", branch=branch
    )
    company_doc = frappe.get_doc("Company", company)
