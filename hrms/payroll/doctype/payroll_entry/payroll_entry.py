...
class PayrollEntry(Document):
    ...
    def before_submit(self):
        ...
        if self.get_employees_with_unmarked_attendance():
            frappe.throw(
                _("Cannot submit. Attendance is not marked for some employees.")
            )
    ...
    def validate_existing_salary_slips(self):
        ...
        if len(existing_salary_slips):
            ...
            msg += _("Reference: {0}").format(
                comma_and(
                    [
                        get_link_to_form("Salary Slip", d.name)
                        for d in existing_salary_slips
                    ]
                )
            )
            frappe.throw(
        ...
        if frappe.db.get_value("Account", self.payroll_payable_account, "account_type"):
            frappe.throw(
                _(
                    ...
                    frappe.bold(
                        get_link_to_form("Account", self.payroll_payable_account)
                    )
                )
            )
    ...
    def get_linked_salary_slips(self):
        return frappe.get_all(
            "Salary Slip", {"payroll_entry": self.name}, ["name", "docstatus"]
        )
    ...
    def update_employees_with_withheld_salaries(self):
        withheld_salaries = get_salary_withholdings(
            self.start_date, self.end_date, pluck="employee"
        )
            ...
            salary_components = (
                ...
                .where(
                    ...
                    & (
                        ...
                        | (
                            (ssd.do_not_include_in_total == 1)
                            & (ssd.do_not_include_in_accounts == 0)
                        )
                    )
                )
            ).run(as_dict=True)
    ...
    def should_add_component_to_accrual_jv(
        self, component_type: str, item: dict
    ) -> bool:
    ...
    def set_employee_based_payroll_payable_entries(
        ...
        employee_details = self.employee_based_payroll_payable_entries.setdefault(
            employee, {}
        )
        ...
        if not self.employee_cost_centers.get(employee):
            ...
            cost_centers = dict(
                (
                    ...
                    .select(
                        EmployeeCostCenter.cost_center, EmployeeCostCenter.percentage
                    )
    ...
    def make_journal_entry(
        ...
        journal_entry.party_not_required = (
            True if not employee_wise_accounting_enabled else False
        )
    ...
    @if_lending_app_installed
    def process_loan_repayments_for_bank_entry(
        self, salary_details: list[dict]
    ) -> float:
        unique_salary_slips = {row["employee"]: row for row in salary_details}.values()
        total_loan_repayment = sum(
            flt(slip.get("total_loan_repayment", 0)) for slip in unique_salary_slips
        )
    ...
    def set_journal_entry_in_salary_slips(self, submitted_salary_slips, jv_name=None):
        ...
        (
            ...
            .where(
                SalarySlip.name.isin(
                    [salary_slip.name for salary_slip in submitted_salary_slips]
                )
            )
        ).run()
    ...
    def get_payroll_dates_for_employee(self, employee_details: dict) -> tuple[str, str]:
        ...
        if (
            ...
            and employee_details.relieving_date < getdate(self.end_date)
        ):
    ...
    def get_holidays_count(
        self, holiday_list: str, start_date: str, end_date: str
    ) -> float:
...
def get_month_details(year, month):
    ...
    if ysd:
        ...
        month_days = cint(
            calendar.monthrange(cint(msd.year), cint(month))[1]
        )  # days in month
...
def create_salary_slips_for_employees(employees, args, publish_progress=True):
    ...
    try:
        ...
        frappe.publish_realtime(
            "completed_salary_slip_creation", user=frappe.session.user
        )
    ...
    if not submitted and not unsubmitted:
        ...
        frappe.msgprint(
            _("Could not submit some Salary Slips: {}").format(
                ", ".join(
                    get_link_to_form("Salary Slip", entry) for entry in unsubmitted
                )
            ),
            title=_("Failure"),
            indicator="red",
...
def submit_salary_slips_for_employees(
    payroll_entry, salary_slips, publish_progress=True
):
    try:
        submitted = []
        unsubmitted = []
...
def get_employee_list(
    ...
    return remove_payrolled_employees(
        employees_to_check, filters.start_date, filters.end_date
    )
