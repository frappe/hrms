...
class EmployeeTaxExemptionProofSubmission(Document):
	...
	def set_total_exemption_amount(self):
		self.exemption_amount = flt(
			get_total_exemption_amount(self.tax_exemption_proofs),
			self.precision("exemption_amount"),
		)

	def calculate_hra_exemption(self):
		(
			self.monthly_hra_exemption,
			self.monthly_house_rent,
			self.total_eligible_hra_exemption,
		) = (0, 0, 0)
		if self.get("house_rent_payment_amount"):
			...
			if hra_exemption:
				...
				self.monthly_hra_exemption = flt(
					hra_exemption["monthly_exemption"],
					self.precision("monthly_hra_exemption"),
				)
				self.monthly_house_rent = flt(
					hra_exemption["monthly_house_rent"],
					self.precision("monthly_house_rent"),
				)
