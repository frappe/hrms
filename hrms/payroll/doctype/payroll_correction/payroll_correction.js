frappe.ui.form.on("Payroll Correction", {
    ...
    additional_salary_date(frm) {
        if (frm.doc.additional_salary_date && frm.doc.additional_salary_date < frappe.datetime.nowdate()) {
            frappe.msgprint({
    ...
    load_lwp_months(frm) {
        if (!(frm.doc.employee && frm.doc.payroll_period && frm.doc.company)) {
            ...
            ["salary_slip_reference", "absent_days", "working_days", "lwp_days", "total_lwp_applied"].forEach(f => frm.set_value(f, undefined));
            return;
        }
            ...
            callback(res) {
                ...
                slips.forEach(d => {
                    if ((d.absent_days || 0) > 0 || (d.leave_without_pay || 0) > 0) {
                        let month_name = new Date(d.posting_date).toLocaleString('default', { month: 'long' });
                ...
                frm.set_df_property("select_month_for_lwp_reversal", "options", [""].concat([...month_set].sort()).join("\n"));
    ...
    select_month_for_lwp_reversal(frm) {
        let selected_entry = frm.lwp_array.find(e => e.month_name === frm.doc.select_month_for_lwp_reversal);
        ...
        if (selected_entry) {
            ...
            frm.set_value("total_lwp_applied", selected_entry.absent_days + selected_entry.leave_without_pay);
        }
    ...
    days_to_reverse(frm) {
        ...
        if (value > total) {
            frappe.msgprint({
                title: __("Invalid Number of Days"),
                message: __("The number of days planned for reversal cannot exceed the total LWP days."),
                indicator: "red"
            });
