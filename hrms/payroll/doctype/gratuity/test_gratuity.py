...
class TestGratuity(IntegrationTestCase):
    def setUp(self):
        ...
        make_earning_salary_component(
            setup=True, test_tax=True, company_list=["_Test Company"]
        )
        make_deduction_salary_component(
            setup=True, test_tax=True, company_list=["_Test Company"]
        )
    ...
    def test_gratuity_based_on_current_slab_via_additional_salary(self):
        ...
        gratuity = create_gratuity(
            pay_via_salary_slip=1, employee=self.employee, rule=rule.name
        )
        ...
        self.assertTrue(
            frappe.db.exists("Additional Salary", {"ref_docname": gratuity.name})
        )
            ...
            new_slab = frappe.get_doc(
                {
                    ...
                    "fraction_of_applicable_earnings": slab[
...
def create_gratuity(**args):
    ...
    if gratuity.pay_via_salary_slip:
        ...
        gratuity.payable_account = args.payable_account or get_payable_account(
            "_Test Company"
        )
