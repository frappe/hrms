...
class IncomeTaxComputationReport:
                ...
                if tax_slab and not tax_slab.disabled:
                    employee_ss_assignments.setdefault(
                        d.employee,
                        {
                            ...
                            "taxable_earnings_till_date": d.taxable_earnings_till_date
                            or 0.0,
    ...
    def get_future_salary_slips(self):
        ...
        for employee in list(self.employees.keys()):
                ...
                last_ss = frappe._dict(
                    {
                        ...
                        "salary_structure": self.employees[employee].get(
                            "salary_structure"
                        ),
                    }
                )
            ...
            while getdate(ss_start_date) < getdate(self.payroll_period_end_date) and (
                ...
                ss_end_date = get_start_end_dates(
                    last_ss.payroll_frequency, ss_start_date
                ).end_date
                ...
                try:
                    ...
                    self.future_salary_slips.setdefault(employee, []).append(
                        ss.as_dict()
                    )
                except Exception:
                    break
    ...
    def get_gross_earnings(self):
        ...
        for employee, employee_details in self.employees.items():
            ...
            gross_earnings = (
                flt(opening_taxable_earnings)
                + flt(existing_ss.get(employee))
                + future_ss_earnings
            )
    ...
    def get_tax_exempted_components(self):
        ...
        tax_exempted_components = (
            nontaxable_earning_components + tax_exempted_deduction_components
        )
    ...
    def get_employee_tax_exemptions(self):
        # add columns
        exemption_categories = frappe.get_all(
            "Employee Tax Exemption Category", {"is_active": 1}
        )
    ...
    def get_tax_exemptions(self, source):
        ...
        records = (
            ...
            .select(
                par.employee, child.exemption_category, Sum(child.amount).as_("amount")
            )
        ...
        for d in records:
            ...
            if (
                source == "Employee Tax Exemption Declaration"
    ...
    def get_hra(self):
        if not frappe.get_meta("Employee Tax Exemption Declaration").has_field(
            "monthly_house_rent"
        ):
            return
    ...
    def get_total_taxable_amount(self):
        ...
        for employee, emp_details in self.employees.items():
            ...
            annual_taxable_amount = tax_exemption_declaration = (
            ...
            if last_ss and last_ss.end_date == self.payroll_period_end_date:
                ...
                if future_salary_slips:
                    ...
                    tax_exemption_declaration = last_ss.get(
                        "tax_exemption_declaration", 0.0
                    )
                    standard_tax_exemption_amount = last_ss.get(
                        "standard_tax_exemption_amount", 0.0
                    )
                    deductions_before_tax_calculation = last_ss.get(
                        "deductions_before_tax_calculation", 0.0
                    )
    ...
    def get_data_for_eval(self, emp: str, emp_details: dict) -> tuple:
        ...
        if last_ss:
            ...
            salary_slip.start_date = max(
                self.payroll_period_start_date, emp_details.date_of_joining
            )
    ...
    def get_total_deducted_tax(self):
        ...
        for d in records:
            total_tax_deducted = (
                flt(self.employees[d.employee].get("tax_deducted_till_date", 0))
                + d.amount
            )
            self.employees[d.employee].setdefault(
    ...
    def get_payable_tax(self):
        ...
        for __, emp_details in self.employees.items():
            payable_tax = flt(emp_details.get("applicable_tax")) - flt(
    ...
    def add_column(
        self, label, fieldname=None, fieldtype=None, options=None, width=None
    ):
        col = {
            "label": _(label),
