...
var I = (o, u, s) =>
	new Promise((j, m) => {
		var f = (p) => {
			...
			q = (p) => (p.done ? j(p.value) : Promise.resolve(p.value).then(f, b));
		q((s = s.apply(o, u)).next());
	});
import { _ as H } from "./RequestField-B8HqhQXt.js";
import {
	c as V,
	v as me,
	o as r,
...
import { b as ye } from "./index-BpEXs0_U.js";
import { a as M, n as Z, e as be } from "./toast-Ca-cKV9o.js";
const F = (o) => `hrms.api.employee_requests.${o}`,
	xe = V({ url: F("get_form") }),
	Je = V({ url: F("get_request") }),
	he = V({ url: F("create_request") }),
	ke = V({ url: F("update_request") }),
	Pe = V({ url: F("delete_request") });
function Xe(o, u) {
	return `/requests/${o}/${u}`;
}
const _e = { key: 0, class: "flex items-center gap-2 py-6 text-base text-ink-gray-5" },
	...
	Ve = ["href"],
	Fe = { class: "truncate" },
	je = { key: 1, class: "flex flex-col gap-1.5" },
	Ne = { class: "flex min-w-0 items-center gap-2" },
	...
	Ye = {
		...
		setup(o, { emit: u }) {
			const s = o,
				...
				O = w({ get: () => s.open, set: (t) => j("update:open", t) }),
				ee = w(() => {
					var e;
				...
				z = w(() => q.value.filter((t) => !p.value.includes(t.name))),
				D = w(() => {
					const t = c.value;
					if (!t) return null;
			...
			function se(t) {
				for (const e of t.target.files) {
					if (z.value.length + b.value.length >= L) {
						M("Too many files", `You can attach at most ${L} files.`);
						break;
					}
					if (e.size > U * 1024 * 1024) {
						M("File too large", `${e.name} is over ${U} MB.`);
						continue;
					}
					b.value.push(e);
				}
