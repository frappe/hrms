		...
		<template v-if="d">
			...
			<Tabs v-model="tab">
				<TabList variant="underline">
					<TabTrigger v-for="t in TABS" :key="t.key" :value="t.key" :label="t.label" />
				</TabList>
				<!-- ABOUT -->
				<TabPanel value="about" class="grid items-start gap-3.5 pt-3.5 lg:grid-cols-2">
					<div class="flex flex-col gap-3.5">
						<SectionCard title="Personal" action="Edit" @action="edit('personal')">
							<dl class="flex flex-col">
								...
								<FieldRow label="Blood group" :value="d.personal.blood_group" />
								<FieldRow
						...
						<SectionCard title="Addresses" action="Edit" @action="edit('addresses')">
							<dl class="flex flex-col">
								<FieldRow label="Current" :value="d.addresses.current_address" />
								<FieldRow
									label="Permanent"
				...
				<!-- JOB -->
				<TabPanel value="job" class="grid items-start gap-3.5 pt-3.5 lg:grid-cols-2">
					<div class="flex flex-col gap-3.5">
						...
						<SectionCard title="History" :padded="false">
							<ul class="px-3.5 pb-3.5">
								...
								>
									...
									<span class="text-p-base text-ink-gray-8">{{ h.label }}</span>
								</li>
							</ul>
						</SectionCard>
					</div>
					...
					<div class="flex flex-col gap-3.5">
						<SectionCard title="Reporting">
							...
							>
								<PersonRow
									...
									:meta="`${d.reporting.manager.designation || ''} · Manager`"
								...
								<PersonRow
									...
									:meta="`${d.reporting.skip.designation || ''} · Skip-level`"
						...
						<SectionCard v-if="d.reporting.peers.length" title="Team">
							...
							<div class="flex flex-wrap items-center gap-1.5">
								...
								>
									<Avatar :label="p.employee_name" :image="p.image" size="lg" />
								</RouterLink>
				...
				<!-- PAY -->
				<TabPanel value="pay" class="grid items-start gap-3.5 pt-3.5 lg:grid-cols-2">
						...
						<SectionCard title="Tax">
							...
							<dl class="flex flex-col">
								...
								<FieldRow
									label="Status"
									:value="
										d.pay.tax.has_declaration ? 'Submitted' : 'Not declared'
									"
								/>
							</dl>
						</SectionCard>
				...
				<!-- TIME -->
				<TabPanel value="time" class="grid items-start gap-3.5 pt-3.5 lg:grid-cols-2">
					<div class="flex flex-col gap-3.5">
					...
					<SectionCard title="Shift and Calendar" readonly-label="HR-owned">
						<dl class="flex flex-col">
							<FieldRow label="Assigned shift" :value="d.time.shift?.name" locked />
							<FieldRow
								label="Timing"
							...
							<FieldRow label="Holiday list" :value="d.time.holiday_list" locked />
						</dl>
					</SectionCard>
				</TabPanel>

				<!-- DOCUMENTS -->
				<TabPanel value="documents" class="grid items-start gap-3.5 pt-3.5 lg:grid-cols-2">
...
import { Avatar, Button, TabList, TabPanel, Tabs, TabTrigger } from "frappe-ui";
...
// v1 Tabs models the trigger value, which is already the key the URL carries,
// so the tab and the query string stay in step without a translation layer.
watch(tab, (key) => {
